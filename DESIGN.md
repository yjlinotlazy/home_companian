...
服务器在生成 Kindle 待投递 Frame 时，原地覆盖 `library/rendered/<device-id>.png`，保留 Dropbox 中同一文件的身份和共享链接；不得先删除、重建或用临时文件原子替换。这里保存的是供物理屏幕直接显示的最终 600×800 PNG；横屏任务板已经完成旋转，竖屏寻宝图保持竖向。清单、字体、显示模式、寻宝状态或手动刷新发生变化时都会重新生成。Kindle 优先请求家庭服务器，失败时可使用本地 `client.conf` 中的 `REMOTE_IMAGE_URL` 下载 Dropbox 等 HTTPS 静态链接，并允许 HTTP 重定向。完整共享链接本身等同访问密钥；不得写入仓库、示例配置或日志。Dropbox 中只保存最终屏幕文件，不保存位置或其他输入数据。静态下载没有 Frame ACK。
