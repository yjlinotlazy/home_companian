...
- [x] 灰度设备保留缩放抗锯齿，单色设备只在最终 encoder 阶段二值化。
...
- [x] 定义 `math/problems.csv` 的算术与数学思维题；24 点改为运行时随机生成并验证有整数解。
- [x] 数学画面只显示题目，保留但不暴露答案。
- [x] 两个模块均支持随机选择并避免连续重复。
- [x] 支持把模板与模块组合成多个 panel 一起随机轮换；CrowPanel 在三栏内容和全屏 24 点之间选择。
...
- [x] 使用 `checklists.csv` 定义带稳定递增 ID 的任务，并由 `config.yaml` 定义清单归属与顺序。
- [x] 使用 `checklist_completions.csv` 保存每日完成结算，跨日自动显示为未完成但保留历史。
- [x] 网页支持勾选和取消，并更新设备下一帧缩略图。
- [x] 添加 Kindle 四区域模板：三个清单区和一个 `images/etc` 随机图区。
- [x] Kindle 只显示服务端 Frame，不实现清单互动。
- [x] 个人任务积 1 分、家庭任务积 2 分，两个人共用一个 50 分封顶余额。
- [x] 玩具奖励初始赠送 25 分；网页满分后手动兑换，记录历史并将下一轮归零。
- [x] 在“家”区域底部显示无数字的玩具进度条。

验收：网页勾选后，当前设备预览不变；Kindle 下一次唤醒取得的新 Frame 显示对应 checkbox，历史完成记录仍可用于未来统计。
